...
var reserveDate = "";
	$(document).ready(function() {
						...
						console.log(Trainno + "," + reserveDate);
						...
						$.ajax({
									...
									success : function(arrayList) {
										...
										for (var i = 0; i < 14; i++) {
											output += '<tr>';
											output += '<th class="seatBtn-Th" id="'
													+ arr1[i]
													+ '" onclick="return seatBtn(this)">'
													+ arr1[i] + '</th>';
											output += '<th class="seatBtn-Th" id="'
													+ arr2[i]
													+ '"onclick="return seatBtn(this)">'
						...
						$.ajax({
									...
									url : "${pageContext.request.contextPath }/getResevedSeat",
									success : function(seatData) {
										console.log("예약된 좌석 : " + seatData);
										var seatList = seatData.split(",");
...
<body>
	...
	<div class="col-12">
		<div style="margin-bottom: 10px; text-align: center;">
			<button id="previousBtn" type="button" class="btn btn-danger"
				onclick="return previousBtn(this)" style="float: left;">이전</button>
			<div style="display: inline-block;border: 1px solid gray; border-radius: 5px; padding-top: 3px; padding-bottom: 3px; padding-left: 10px; padding-right: 10px;">
  <h4 id="carCaption" style="display: inline-block; margin-bottom: 0px;">5호차</h4>
</div>
			<button id="nextBtn" type="button" class="btn btn-danger"
				onclick="nextBtn(this)" style="float: right;">다음</button>
			<input type="hidden" value="5" id="carNum">
		</div>
		<div class="card">
			<div class="card-body" id="outputSeatNum"></div>
			<button id="nextBtn" type="button" class="btn btn-danger"
				onclick="selectComplete()" style="float: center;">선택완료</button>
		</div>
	</div>
		...
		$.ajax({
					...
					success : function(arrayList) {
						...
						if (arrayList.length > 50) {
							...
							for (var i = 0; i < 14; i++) {
								output += '<tr>';
								output += '<th class="seatBtn-Th" id="'+arr1[i]+'" onclick="return seatBtn(this)">'
										+ arr1[i] + '</th>';
								output += '<th class="seatBtn-Th" id="'+arr2[i]+'" onclick="return seatBtn(this)">'
										+ arr2[i] + '</th>';
								output += '<th></th>';
								output += '<th class="seatBtn-Th" id="'+arr3[i]+'" onclick="return seatBtn(this)">'
										+ arr3[i] + '</th>';
								output += '<th class="seatBtn-Th" id="'+arr4[i]+'" onclick="return seatBtn(this)">'
										+ arr4[i] + '</th>';
								output += '</tr>';
